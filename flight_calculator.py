...
def generate_grid_path(area_width, area_height, overlap, sidelap, altitude, speed, interval, fov, center_lat=None, center_lon=None, direction='north_south'):
    ...
    # Create reference point at the top-left corner
    width_offset = area_width / 2
    height_offset = area_height / 2

    if direction == 'north_south':
        # North-South pattern: Move west for width offset, then north for height
        top_left = geodesic(kilometers=width_offset/1000).destination(
            point=(center_lat, center_lon), 
            bearing=270  # West
        )
        top_left = geodesic(kilometers=height_offset/1000).destination(
            point=top_left, 
            bearing=0  # North
        )
    else:
        # East-West pattern: Move north for width offset, then west for height
        top_left = geodesic(kilometers=width_offset/1000).destination(
            point=(center_lat, center_lon), 
            bearing=0  # North
        )
    ...
    meters_per_lat = 111320  # Approximate meters per degree of latitude
